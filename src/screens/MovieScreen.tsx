        ...
        <TextContainer>
          <View style={{flexDirection:'row',width:'70%',justifyContent:'flex-end'}}>
            ...
            <ButtonStyled onPress={()=>toogleFavorite(infoMovie,isFavorite,change)}>
              <MaterialIcons name={(isFavorite)?"favorite":"favorite-outline"} size={24} color={(isFavorite)?"#F72585":"white"} />
            </ButtonStyled>
          ...
          <TextGenre>Categorias: {infoMovie.Genre}</TextGenre>
...
const message=(title,plot)=>(`¿Has visto ${title}? trata sobre ${plot}`)
